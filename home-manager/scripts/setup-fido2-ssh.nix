...
let
  setup-fido2-ssh = pkgs.writeShellScriptBin "setup-fido2-ssh" ''
    set -e
    ...
    if [ ! -f ~/.ssh/fido2 ]; then
      ...
      ssh-keygen -t ed25519-sk -f ~/.ssh/fido2 -C "jtekk@yubikey-fido2"
    else
    ...
    if [ ! -f ~/.ssh/fido2_ecdsa ]; then
      ...
      if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        echo "Touch your YubiKey when prompted!"
        ssh-keygen -t ecdsa-sk -f ~/.ssh/fido2_ecdsa -C "jtekk@yubikey-ecdsa"
      fi
    else
